...
-- Create a table to store player state tracking
...
-- Insert data into the players_state_tracking table for the initial season
WITH initial_season AS (
  SELECT player_name, season AS first_active_season, season AS last_active_season, 'Active' AS player_state, season AS current_season
  FROM player_seasons
  WHERE season = 1996
)
INSERT INTO players_state_tracking (player_name, first_active_season, last_active_season, player_state, current_season)
SELECT player_name, first_active_season, last_active_season, player_state, current_season
FROM initial_season;

-- Update the players_state_tracking table for subsequent seasons
WITH last_season AS (
  SELECT player_name, first_active_season, last_active_season, player_state, current_season
...
SELECT 
  ...
  COALESCE(l.first_active_season, t.season) AS first_active_season,
  COALESCE(t.season, l.last_active_season) AS last_active_season,
  CASE
    WHEN l.player_name IS NULL THEN 'New'
    WHEN t.player_name IS NULL THEN 'Retired'
    WHEN l.last_active_season = t.season - 1 AND l.player_state = 'Active' THEN 'Continued Playing'
    WHEN l.last_active_season < t.season - 1 AND l.player_state = 'Retired' THEN 'Returned from Retirement'
    WHEN t.player_name IS NULL AND l.player_state = 'Retired' THEN 'Stayed Retired'
    ELSE 'Unknown'
  END AS player_state,
